cd /home/leo_zhang/Documents/GitHub && \
git clone git@github.com:leozhang1/AmazonPriceTrackerTool.git && \
git clone git@github.com:leozhang1/bmbl_bot.git && \
git clone git@github.com:leozhang1/DroneFun.git && \
git clone git@github.com:leozhang1/easy-apply-linkedin.git  && \
git clone git@github.com:leozhang1/FileManagementTool.git && \
git clone git@github.com:leozhang1/FlightScraper.git && \
git clone git@github.com:leozhang1/GeneralDB.git && \
git clone git@github.com:leozhang1/GetDailyPic.git && \
git clone git@github.com:leozhang1/GithubAutomation.git  && \
git clone git@github.com:leozhang1/important_docs.git  && \
git clone git@github.com:leozhang1/Journal.git  && \
git clone git@github.com:leozhang1/LeoZhang.git && \
git clone git@github.com:leozhang1/music_collection.git  && \
# git clone git@github.com:leozhang1/my_linux_configs.git && \
git clone git@github.com:leozhang1/NewsScraper.git && \
git clone git@github.com:leozhang1/okcupidbot.git && \
git clone git@github.com:leozhang1/PasswordManagerNodeJs.git && \
git clone git@github.com:leozhang1/PersonalPortfolioFlutterWeb.git && \
git clone git@github.com:leozhang1/PongGame.git && \
git clone git@github.com:leozhang1/practice_interview_problems.git && \
git clone git@github.com:leozhang1/summer_2022_class.git && \
git clone git@github.com:leozhang1/TaxLienScraper.git && \
git clone git@github.com:leozhang1/tdr_bot.git && \
git clone git@github.com:leozhang1/Tic_Tac_Toe.git && \
git clone git@github.com:leozhang1/TruistCreditCardManager.git && \
git clone git@github.com:leozhang1/Zoom-Automation-Python.git


